...
def show_predictions(raw, gt, pred, prediction_type):
    """
    Show raw input, ground truth, and predicted segmentation.

    Args:
        raw (numpy.ndarray): Input images (B, C, H, W)
        gt (numpy.ndarray): Ground truth labels (B, 1, H, W) or (B, H, W)
        pred (numpy.ndarray): Model predictions (B, C, H, W)
        prediction_type (PredictionType): TWO_CLASS or THREE_CLASS
    """
    num_samples = pred.shape[0]

    # Convert prediction to class map
    if prediction_type == PredictionType.TWO_CLASS:
        pred_map = (pred[:, 0] >= 0.9).astype(np.uint8)  # thresholded foreground
        gt_map = gt[:, 0] if gt.ndim == 4 else gt
    elif prediction_type == PredictionType.THREE_CLASS:
        pred_map = np.argmax(pred, axis=1)  # argmax over channel dimension
        gt_map = gt[:, 0] if gt.ndim == 4 else gt
    else:
        raise NotImplementedError("Unsupported prediction type.")

    fig, ax = plt.subplots(num_samples, 3, sharex=True, sharey=True, figsize=(12, num_samples * 4))

    for i in range(num_samples):
        ax[i, 0].imshow(raw[i, 0], cmap="gray")
        ax[i, 1].imshow(gt_map[i], cmap="gist_earth", vmin=0, vmax=2 if prediction_type == PredictionType.THREE_CLASS else 1)
        ax[i, 2].imshow(pred_map[i], cmap="gist_earth", vmin=0, vmax=2 if prediction_type == PredictionType.THREE_CLASS else 1)
...
def old_show_predictions(raw, gt, pred):
...
def show_instance_segmentation(image, gt_labels, surface, instance_segmentation):
    ...
    ax.set_title("watershed surface")
    plt.imshow(np.squeeze(1.0 - surface))

    ax = fig.add_subplot(1, 4, 4)
    ax.set_title("pred instance segmentation")
