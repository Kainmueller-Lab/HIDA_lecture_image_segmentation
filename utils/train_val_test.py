...
def calc_accuracy(y_pred, y_true):
    """
    Calculate pixel-wise accuracy for 2+ class segmentation using argmax.

    Args:
        y_pred (torch.Tensor): Predicted logits or probabilities. Shape [B, C, H, W]
        y_true (torch.Tensor): Ground truth labels. Shape [B, H, W] or [B, 1, H, W]
...
def old_calc_accuracy(y_pred, y_true):
    ...
    with tqdm(total=training_steps) as pbar:
        ...
        while step < training_steps:
            ...
            for img, label in tmp_loader:
                # prepare data
                dtype = torch.FloatTensor if prediction_type == PredictionType.TWO_CLASS else torch.LongTensor
